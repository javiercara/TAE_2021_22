...
Las funciones que calculan el logaritmo de la verosimilitud, el gradiente y el hessiano se han incluido en el archivo [poisson_funciones.R](poisson_funciones.R).
...
grad_logL(beta,y,X)
hess_logL(beta, X)
nlme::fdHess(beta,logL, y = d$damage, X)
```
...
beta_i = coef(m)
X = model.matrix(m)
mle = optim(par = beta_i, fn = logL_optim, y = d$damage, X = X, gr = NULL, method = "BFGS", hessian = TRUE, control = list(trace=1, REPORT = 1, maxit = 200))
mle$par
```
