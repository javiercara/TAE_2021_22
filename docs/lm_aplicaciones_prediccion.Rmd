...
Por tanto $\hat y_p$ es la predicción del valor medio en $x_p$.
...
\end{bmatrix}
=
\hat \beta_0 + \tilde x_p^T \hat \beta^*
$$

Por otro lado hemos visto que

$$
\bar y = \beta_0 + \beta_1 \bar x_{1} + \beta_2 \bar x_{2} + \cdots + \beta_k \bar x_{k} = \hat \beta_0 + \bar x^T \hat \beta^*
$$

donde $\bar x^T = [\bar x_{1} \ \bar x_{2} \ \cdots \ \bar x_{k}]$. Despejando $\hat \beta_0$ y sustituyendo en la otra ecuación se obtiene

$$
\hat y_p = \bar y + (\tilde x_p - \bar x)^T \hat \beta^*
$$

Por tanto:

$$
Var[\hat y_p] = Var[\bar y] + (\tilde x_p - \bar x)^T Var[\hat \beta^*] (\tilde x_p - \bar x)
$$
...
El valor real para $x_p$ es $y_p$ y no $\hat y_p$. Por tanto tenemos un error de predicción:
...
Por tanto podemos averiguar la distribución de $\epsilon_p = y_p - \hat y_p$:
...
En este caso no se puede utilizar bootstrap. El método bootstrap se utiliza para calcular intervalos de confianza y se basa en remuestrear residuos. E $y_p$ es una variable aleatoria, no unos residuos. 